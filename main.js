...
const _revenda = JSON.parse(fs.readFileSync('./datab/usuarios/servidores.json'));
const limitefll = JSON.parse(fs.readFileSync('./datab/usuarios/flood.json'));
const servers = JSON.parse(fs.readFileSync('./datab/usuarios/_revenda.json'));
const subs = JSON.parse(fs.readFileSync('./datab/usuarios/subs.json'));
...
const { nomeBot, numerodono, keykauan, server_ofc, senha_ofc } = setting
const ownerNumber = [numerodono+"@s.whatsapp.net","554491150998@s.whatsapp.net"]
NomeDoBot = nomeBot
...
var prefix = setting.prefix 
        ...
        //antis
        const subRevenda = subs.map(i => i.id).includes(sender)
 ...
 * @param {object} _premi
 */
const addRevendaUser = (userId, expired, _premi, login, server, senha) => {
    const obj = { id: userId, server: server, senha: senha, server_ofc: true, login: login, sub: [], expired: Date.now() + toMs(expired) }
    _premi.push(obj)
    fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_premi))
}

const getPositionSubRevenda = (sender) => {
position = subs.map(i => i.id).indexOf(sender)
ba = subs[position].revenda
revenda = _revenda.map(i => i.id).indexOf(ba)
return revenda
}
...
const expiredCheck = (_dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_dir))
        }
    }, 1000)
}

const addsubRevendaUser = (owner, userId, expired, _premi, login) => {
    const obj = { id: userId, revenda: owner, expired: Date.now() + toMs(expired) }
    _premi.push(obj)
    fs.writeFileSync('./datab/usuarios/subs.json', JSON.stringify(_premi))
}
...
const expiredChecksub = (_dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            console.log(`Revenda user expired: ${_dir[position].id}`)
            _dir.splice(position, 1)
            fs.writeFileSync('./datab/usuarios/subs.json', JSON.stringify(_dir))
        }
    }, 1000)
}
...
function deletelogin(user, valor, userId){
if(isRevenda){
position = getRevendaPosition(user, _revenda)
_revenda[position].login += valor
fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda))
}else if(subRevenda){
position = getRevendaPosition(userId, _revenda)
posi = _revenda[position].sub.map(i => i.id).indexOf(user)
_revenda[position].sub[posi].login += valor
fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda))
}}
...
if(subRevenda){
position = subs.map(i => i.id).indexOf(sender)
if(subs[position].login < 1)return reply('Desculpe mas seus logins expirarão')
}else if (isRevenda){
position = _revenda.map(i => i.id).indexOf(sender)
if(_revenda[position].senha == "" || _revenda[position].server == "" ))return reply('Desculpe defina seu servidor primeiro\nUsando '+`${prefix}server {server} {senha}\nExemplo:\n${prefix}server 168.164.135.12 1836ga45`)
}
}

        switch (command) {
case 'server':
if(!args[0])return reply(`Informe o servidor\nExemplo:\n${prefix+command} 192.168.43.1 senha`)
 if(!args[1])return reply(`Informe a senha\nExemplo:\n${prefix+command} ${args[0]} senha`)
if(!isOwner && !isRevenda)return reply("somente revendedores")
position = _revenda.map(i => i.id).indexOf(sender)
if(position >= 0){
_revenda[position].server = args[0]
_revenda[position].senha = args[1]
fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda, null, 2))
reply("Servidor atualizado com sucesso")
}else{
_revenda.push({
id: sender,
server: args[0],
...
fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda, null, 2))
reply("Servidor definido com sucesso")
}
break
        
  case 'sshgratis':
  if (!isRevenda && !subRevenda && !isOwner) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
if(!isOwner)checkLogin(sender)
position = _revenda.map(i => i.id).indexOf(sender)
server = subRevenda ? _revenda[getPositionSubRevenda(sender)].server : _revenda[position].server
senha_server = subRevenda ? _revenda[getPositionSubRevenda(sender)].server : _revenda[position].senha

exec('bash ssh/gerarssh.sh'+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
if(stdout.split('✅')[2] == undefined)return reply('Erro ao gerar ssh caso o erro persista informe ao meu dono')
texto = `\n📱TESTE GERADO COM SUCESSO📱


✅ CONECTAMOS VOCÊ COM O MUNDO ✅${stdout.split('✅')[2]}`
...
reply('erro ao gerar login')
        ...
        break      
          ...
          case 'gerarlogin':
          case 'sshlogin':
		if (!isRevenda && !isOwner) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
if(!isOwner)checkLogin(sender)
position = _revenda.map(i => i.id).indexOf(sender)
server = subRevenda ? _revenda[getPositionSubRevenda(sender)].server : _revenda[position].server
senha_server = subRevenda ? _revenda[getPositionSubRevenda(sender)].server : _revenda[position].senha

   exec('bash ssh/gerarlogin.sh'+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
if(stdout.split('✅')[2] == undefined)return reply('Erro ao gerar ssh caso o erro persista informe ao meu dono')
deletelogin(sender, -1)
texto = `📱CRIADO COM SUCESSO📱\n

✅ OBRIGADO PELA PREFERENCIA ✅${stdout.split('✅')[2]}`
conn.sendBT(from, texto, "",[
//{index: 1, urlButton: {displayText: 'Baixar APK ROCKET', url: 'https://play.google.com/store/apps/details?id=com.rocket.net.vpn'}},
], selo)
}else if(err){
reply('erro ao gerar login')
console.log(err)
...
senha_server = subRevenda ? _revenda[getPositionSubRevenda(sender)].server : _revenda[position].senha
   ...
   exec('bash ssh/gerarlogin2.sh'+` ${args[0]} ${args[1]}`+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
if(stdout.split('✅')[2] == undefined)return reply('Erro ao gerar ssh caso o erro persista informe ao meu dono')
deletelogin(sender, -1)
texto = `📱CRIADO COM SUCESSO📱\n
...
case 'sshlista':
        case 'sshlist':
	if (!isRevenda && !isOwner || subRevenda) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
if(!isOwner)checkLogin(sender)
position = _revenda.map(i => i.id).indexOf(sender)
server = isOwner ? server_ofc : _revenda[position].server
senha_server = isOwner ? senha_ofc : _revenda[position].senha
exec('bash ssh/online.sh'+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
txt = ''
...
server = isOwner ? server_ofc : _revenda[position].server
senha_server = isOwner ? senha_ofc : _revenda[position].senha
exec('bash ssh/sshlimit.sh'+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
txt = ``
...
server = isOwner ? server_ofc : _revenda[position].server
senha_server = isOwner ? senha_ofc : _revenda[position].senha
exec('bash ssh/sshmonitor.sh'+` ${server} ${senha_server}`, (err, stdout) => {
if (stdout){
...
conn.sendBT(from, tx, "",[
{index: 1, quickReplyButton: {displayText: 'LISTA ONLINES', id: `${prefix}lista-onlines`}},    
...
📱 STATUS SSH - statusssh

📱 ACESSO MANUAL - criarlogin {user} {senha} 

📱 VENCIMENTO E SALDO - meusaldo 

*OBS. NÃO CRIE LOGIN NEM SENHA COM MENOS DE 6 DIGITIOS 
SUJEITO A ERRO DO SISTEMA* 

`
switch(args[0]){
case '2':
conn.sendBT(from, texto, "",[
{index: 1, urlButton: {displayText: 'SUPORTE SKYNERD', url: 'https://t.me/batmonn'}},
...
{index: 3, quickReplyButton: {displayText: 'VENCIMENTO E SALDO', id:`${prefix}meusaldo`}}
], selo)
break
default:
conn.sendBT(from, texto, "",[
...
case 'revenda':{
							if (args.length === 0) return reply(`Escreva assim *${prefix}revenda* add/del 55xxx tempo (exemplo 1 dia -> 1d) quantidade login\nExemplo:\n${prefix}revenda add 554491150998 1d 10`)
							if (!msg.key.fromMe && !isOwner) return reply("Somente meu dono")
							if(isOwner){
							if (args[0] === 'add') {
								if (msg.message.extendedTextMessage != undefined) {
									mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
									if(_revenda.map(i => i.id).includes(mentioned[0]))return reply("esta pessoa já é revendedor(a)")
									addRevendaUser(mentioned[0], args[2], _revenda, parseInt(args[3]), "", "")
									reply(`*「 REVENDA ADD 」*\n\n📛 *ID*: ${mentioned[0]}\n⏰ *expiração*: ${ms(toMs(args[2])).days} dia(s) ${ms(toMs(args[2])).hours} hora(s) ${ms(toMs(args[2])).minutes} minuto(s)`)
									} else {
									if(_revenda.map(i => i.id).includes(args[1] + "@s.whatsapp.net"))return reply("esta pessoa já é revendedor(a)")
										addRevendaUser(args[1] + '@s.whatsapp.net', args[2], _revenda, parseInt(args[3]), "", "")
										reply(`*「 REVENDA ADD 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net\n⏰ *expiração*: ${ms(toMs(args[2])).days} dia(s) ${ms(toMs(args[2])).hours} hora(s) ${ms(toMs(args[2])).minutes} minutos(s)`)
										}
										} else if (args[0] === 'del') {
											if (msg.message.extendedTextMessage != undefined) {
									mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
									if(getRevendaPosition(mentioned[0], _revenda) == undefined)return reply('desculpe mais esse usuário não é um revendedor')
									_revenda.splice(getRevendaPosition(mentioned[0], _revenda), 1)
									fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda))
									reply(`*「 REVENDA DELETADA 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net`)
									} else {
										if(getRevendaPosition(args[1] + '@s.whatsapp.net', _revenda) == undefined)return reply('desculpe mais esse usuário não é um revendedor')
									    _revenda.splice(getRevendaPosition(args[1] + '@s.whatsapp.net', _revenda), 1)
										fs.writeFileSync('./datab/usuarios/servidores.json', JSON.stringify(_revenda))
										reply(`*「 REVENDA DELETADA 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net`)
										}
										} else {
											reply(`Escreva assim *${prefix}revenda* add/del 55xxx {tempo}(exemplo 1 dia -> 1d) {quantidade de login}\nExemplo:\n${prefix}revenda add 554491150998 31d 10`)
											}
											}else if(isRevenda){
												if (args[0] === 'add') {
								if (msg.message.extendedTextMessage != undefined) {
									mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
									if(mentioned.length == 0)return reply("Mencione alguem")
									if(subs.map(i => i.id).includes(mentioned[0]))return reply("esta pessoa já é subrevendedor(a)")
									addsubRevendaUser(sender, mentioned[0], args[2], subs, parseInt(args[3]))
									reply(`*「 REVENDA ADD 」*\n\n📛 *ID*: ${mentioned[0]}\n⏰ *expiração*: ${ms(toMs(args[2])).days} dia(s) ${ms(toMs(args[2])).hours} hora(s) ${ms(toMs(args[2])).minutes} minuto(s)`)
									} else {
									if(subs.map(i => i.id).includes(args[1] + "@s.whatsapp.net"))return reply("esta pessoa já é revendedor(a)")
										addsubRevendaUser(sender, args[1] + '@s.whatsapp.net', args[2], subs, parseInt(args[3]))
										reply(`*「 REVENDA ADD 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net\n⏰ *expiração*: ${ms(toMs(args[2])).days} dia(s) ${ms(toMs(args[2])).hours} hora(s) ${ms(toMs(args[2])).minutes} minutos(s)`)
										}
										} else if (args[0] === 'del') {
											if (msg.message.extendedTextMessage != undefined) {
									mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
									if(mentioned.length == 0)return reply("Mencione alguem")
									if(getRevendaPosition(mentioned[0], subs) == undefined)return reply('desculpe mais esse usuário não é um revendedor')
									subs.splice(getRevendaPosition(mentioned[0], subs), 1)
									fs.writeFileSync('./datab/usuarios/subs.json', JSON.stringify(subs))
									reply(`*「 REVENDA DELETADA 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net`)
									} else {
										if(getRevendaPosition(args[1] + '@s.whatsapp.net', subs) == undefined)return reply('desculpe mais esse usuário não é um subrevendedor')
									    subs.splice(getRevendaPosition(args[1] + '@s.whatsapp.net', subs), 1)
										fs.writeFileSync('./datab/usuarios/subs.json', JSON.stringify(subs))
										reply(`*「 REVENDA DELETADA 」*\n\n📛 *ID*: ${args[1]}@s.whatsapp.net`)
										}
										} else {
                            ...
                            case 'meusaldo':{
                                if (!isRevenda && !isOwner) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
                                i = _revenda.map(i => i.id).indexOf(sender)
                                checkExp = ms(_revenda[i].expired - Date.now())
                                txt = `🆔 ID : ${sender}\n♨️ Logins: ${_revenda[i].login}\n⏰ Expiração: ${checkExp.days} dia(s) ${checkExp.hours} hora(s) ${checkExp.minutes} minutos(s)\n\n`
                                conn.sendMessage(from,{text: txt, mentions: [sender]}, {quoted: msg})
                                }
                                break
				...
				case 'revendacheck': case 'checkrevenda':{
							if (!isRevenda && !isOwner) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
							cekExp = ms(await getRevendaExpired(sender, _revenda) - Date.now())
							if(isOwner)return reply("Você é meu dono vc pode tudo")
				...
				if(!isOwner)return reply("Somente meu dono")
							...
							let men = subs.filter(i => i.revenda == sender)
							...
							break
                                ...
                                case "revendadias":
                                if (!isOwner) return reply(`Voce não é um revendedor use o comando ${prefix}buyrevenda`)
                                if (!q) return reply(`Escreva assim *${prefix}renovar-revenda* 55xxx tempo (exemplo 1 dia -> 1d)\nExemplo:\n${prefix+command} 554491150998 1d`)
                                if (!args[1]) return reply(`Escreva assim *${prefix}renovar-revenda* ${args[0]} tempo (exemplo 1 dia -> 1d)\nExemplo:\n${prefix+command} ${args[0]} 1d`)
                                if(_revenda.map(i=>i.id).includes(args[0]+"@s.whatsapp.net"))return reply("informe um usuario valido")
                                renovarRevenda(args[0]+"@s.whatsapp.net", args[1])
                                reply(`*「 TEMPO DE REVENDA RENOVADO」*\n\n📛 *ID*: ${args[0]}\n⏰ *tempo add*: ${ms(toMs(args[1])).days} dia(s) ${ms(toMs(args[1])).hours} hora(s) ${ms(toMs(args[1])).minutes} minuto(s)`)
                                break
                                ...
                                position = getRevendaPosition(args[0] + '@s.whatsapp.net', _revenda)
...
case 'buyrevenda':
texto = '*PAINEL SKYNERD SSH*\n\nCredito unitario R$ 2,00 minino para usar o painel 10 unidades.\n*Descrição:* Painel usado no whatsapp agilizando sua venda de internet por VPN\n\n contate responsavel: wa.me/'+numerodono
conn.sendBT(from, texto, "",[
{index: 1, urlButton: {displayText: 'CARTÃO - PIX - TRANSFERENCIA', url: 'https://mpago.la/1ksASjG'}},
], selo)
break
...
if(args.length < 1) return reply('*EEscreva a mensagem de boas-vindas*')
teks = body.slice(11)
if(isWelcomed) {
...
if(args.length < 1) return reply('*EEscreva a mensagem de saída*')
teks = body.slice(13)
if(isByed) {
...
try {
nick = args.join(' ')
if(!nick) return reply('EEscreva o Nick ou nome que você quer personalizar.')
...
///Downloaders
        ...
        case 'criador': {
            ...
            + 'VERSION:3.0\n' 
            + 'FN:MINERD\n' // full name
            + 'ORG:Developer Minerd / Batmonn;\n' // the organization of the contact
            + 'item1.TEL;waid=551149356144:+55 11949356144\n' // WhatsApp ID + phone number
            + 'item1.X-ABLabel:Criador Minerd @batmonn\n'
